...
- New method `<SQLContext>$register_globals()` ([#1064](https://github.com/pola-rs/r-polars/issues/1064)).
- New experimental method `$sql()` for DataFrame and LazyFrame ([#1065](https://github.com/pola-rs/r-polars/issues/1065)).

## Polars R Package 0.16.2

### New features

- `$cut()` and `$qcut()` to bin continuous values into discrete categories ([#1057](https://github.com/pola-rs/r-polars/issues/1057)).
- `pl$scan_parquet()` and `pl$read_parquet()` can read data from the internet by specifying a URL
  to the first argument ([#1056](https://github.com/pola-rs/r-polars/issues/1056), [@andyquinterom](https://github.com/andyquinterom)).
- `pl$scan_parquet()` and `pl$read_parquet()` gain an argument `storage_options`
  ...
  support is experimental ([#1056](https://github.com/pola-rs/r-polars/issues/1056), [@andyquinterom](https://github.com/andyquinterom)).
- Add support for the `Enum` datatype via `pl$Enum()` ([#1061](https://github.com/pola-rs/r-polars/issues/1061)).

### Bug fixes

- In some read/scan functions, downloading files could fail if the URL was too
  long. This is now fixed ([#1049](https://github.com/pola-rs/r-polars/issues/1049), [@DyfanJones](https://github.com/DyfanJones)).

## Polars R Package 0.16.1

This is a small hot-fix release to update dependent Rust polars to 0.39.1 ([#1042](https://github.com/pola-rs/r-polars/issues/1042)).

Also, there are some updates.

### Bug fixes

- `$len()` now correctly includes `null` values in the count ([#1044](https://github.com/pola-rs/r-polars/issues/1044)).

### Other improvements

- `$arr$max()` and `$arr$min()` work without the `nightly` feature ([#1042](https://github.com/pola-rs/r-polars/issues/1042)).

## Polars R Package 0.16.0

### Breaking changes

- Rust polars is updated to 0.39.0 ([#937](https://github.com/pola-rs/r-polars/issues/937), [#1034](https://github.com/pola-rs/r-polars/issues/1034)).
...
- Several functions have been rewritten to match the behavior of Python Polars.
  ...
  1. Change in argument names:
     ...
     - In `pl$read_*` and `pl$scan_*` functions, the first argument is now
       `source` ([#935](https://github.com/pola-rs/r-polars/issues/935)).
     ...
     - In `$str$to_integer()` (renamed from `$str$parse_int()`), argument `radix` is
       renamed to `base` ([#1038](https://github.com/pola-rs/r-polars/issues/1038)).

  2. Change in the way arguments are passed:

     - In all input/output functions, all arguments except the first argument
       must be named arguments ([#935](https://github.com/pola-rs/r-polars/issues/935)).
     - In `<DataFrame>$rolling()` and `<DataFrame>$group_by_dynamic()`, all
       arguments except `index_column` must be named arguments ([#983](https://github.com/pola-rs/r-polars/issues/983)).
     - In `$unique()` for `DataFrame` and `LazyFrame`, arguments `keep` and
       `maintain_order` must be named ([#953](https://github.com/pola-rs/r-polars/issues/953)).
     ...
     - In `$str$contains()`, the arguments `literal` and `strict` must be named
       ([#982](https://github.com/pola-rs/r-polars/issues/982)).
     - In `$str$contains_any()`, the `ascii_case_insensitive` argument must be
       named ([#986](https://github.com/pola-rs/r-polars/issues/986)).
     ...
     - In `$str$to_integer()` (renamed from `$str$parse_int()`), all arguments
       must be named ([#1038](https://github.com/pola-rs/r-polars/issues/1038)).
     - In `pl$date_range()`, the arguments `closed`, `time_unit`, and `time_zone`
       must be named ([#950](https://github.com/pola-rs/r-polars/issues/950)).
     - In `$set_sorted()` and `$sort_by()`, argument `descending` must be named
       ([#1034](https://github.com/pola-rs/r-polars/issues/1034)).
  ...
  3. Arguments removed:
     ...
     - The argument `columns` in `$drop()` is removed. `$drop()` now accepts
       several character scalars, such as `$drop("a", "b", "c")` ([#912](https://github.com/pola-rs/r-polars/issues/912)).
     - In `pl$col()`, the `name` argument is removed, and the `...` argument no
       longer accepts a list of characters and `RPolarsSeries` class objects ([#923](https://github.com/pola-rs/r-polars/issues/923)).
     - In `pl$date_range()`, the unused argument (not working in recent versions)
       `explode` is removed. ([#950](https://github.com/pola-rs/r-polars/issues/950)).

  4. Change in arguments default and accepted values:

     - In `pl$Series()`, the argument `values` has a new default value `NULL`
       ([#966](https://github.com/pola-rs/r-polars/issues/966)).
     - In `$unique()` for `DataFrame` and `LazyFrame`, argument `keep` has a new
       default value `"any"` ([#953](https://github.com/pola-rs/r-polars/issues/953)).
     - In rolling aggregation functions (such as `$rolling_mean()`), the default
       ...
       `window_size` now throws an error ([#937](https://github.com/pola-rs/r-polars/issues/937)).
     - In `pl$date_range()`, the argument `end` must be specified and the default
       ...
       no longer accept numeric values ([#950](https://github.com/pola-rs/r-polars/issues/950)).
     - In `pl$scan_parquet()`, the default value of the argument `rechunk` is
       changed from `TRUE` to `FALSE` ([#1033](https://github.com/pola-rs/r-polars/issues/1033)).
     - In `pl$scan_parquet()` and `pl$read_parquet()`, the argument `parallel`
       ...
       `"none"`, and `"RowGroups"` instead of `"row_groups"` ([#1033](https://github.com/pola-rs/r-polars/issues/1033)).
     - In `$str$to_integer()` (renamed from `$str$parse_int()`), the default
       value of `base` is changed from `2` to `10` ([#1038](https://github.com/pola-rs/r-polars/issues/1038)).
...
- The usage of `pl$date_range()` to create a range of `Datetime` data type is
  ...
  `Datetime` instead ([#950](https://github.com/pola-rs/r-polars/issues/950)).
- `<DataFrame>$get_columns()` now returns an unnamed list instead of a named
  list ([#991](https://github.com/pola-rs/r-polars/issues/991)).
...
- In `$group_by_dynamic()`, the first datapoint is always preserved ([#1034](https://github.com/pola-rs/r-polars/issues/1034)).
- `$str$parse_int()` is renamed to `$str$to_integer()` ([#1038](https://github.com/pola-rs/r-polars/issues/1038)).

### New features

- New functions:

  - `pl$arg_sort_by()` ([#929](https://github.com/pola-rs/r-polars/issues/929)).
  - `pl$arg_where()` to get the indices that match a condition ([#922](https://github.com/pola-rs/r-polars/issues/922)).
  - `pl$datetime()`, `pl$date()`, and `pl$time()` to easily create Expr of class
    datetime, date, and time via columns and literals ([#918](https://github.com/pola-rs/r-polars/issues/918)).
  - `pl$datetime_range()`, `pl$date_ranges()` and `pl$datetime_ranges()` ([#950](https://github.com/pola-rs/r-polars/issues/950), [#962](https://github.com/pola-rs/r-polars/issues/962)).
  - `pl$int_range()` and `pl$int_ranges()` ([#968](https://github.com/pola-rs/r-polars/issues/968))
  - `pl$mean_horizontal()` ([#959](https://github.com/pola-rs/r-polars/issues/959))
  - `pl$read_ipc()` ([#1033](https://github.com/pola-rs/r-polars/issues/1033)).
  - `is_polars_dtype()` ([#927](https://github.com/pola-rs/r-polars/issues/927)).

- New methods:

  - `<LazyFrame>$to_dot()` to print the query plan of a LazyFrame with graphviz
    dot syntax ([#928](https://github.com/pola-rs/r-polars/issues/928)).
  - `$clear()` for `DataFrame`, `LazyFrame`, and `Series` ([#1004](https://github.com/pola-rs/r-polars/issues/1004)).
  - `$item()` for `DataFrame` and `Series` ([#992](https://github.com/pola-rs/r-polars/issues/992)).
  - `$select_seq()` and `$with_columns_seq()` for `DataFrame` and `LazyFrame`
    ([#1003](https://github.com/pola-rs/r-polars/issues/1003)).
  ...
  - `RPolarsDataType` gains several methods to check the datatype, such as
    `$is_integer()`, `$is_null()` or `$is_list()` ([#1036](https://github.com/pola-rs/r-polars/issues/1036)).

- New arguments or argument values:

  - `ambiguous` can now take the value `"null"` to convert ambigous datetimes to
    null values ([#937](https://github.com/pola-rs/r-polars/issues/937)).
  - `n` in `$str$replace()` ([#987](https://github.com/pola-rs/r-polars/issues/987)).
  - `non_existent` in `$dt$replace_time_zone()` to specify what should happen
    when a datetime doesn't exist.
  - `mapping_strategy` in `$over()` ([#984](https://github.com/pola-rs/r-polars/issues/984), [#988](https://github.com/pola-rs/r-polars/issues/988)).
  - `raise_if_undetermined` in `$meta$output_name()` ([#961](https://github.com/pola-rs/r-polars/issues/961)).
  - `null_on_oob` in `$arr$get()` and `$list$get()` to determine what happens
    when the index is out of bounds ([#1034](https://github.com/pola-rs/r-polars/issues/1034)).
  - `nulls_last`, `multithreaded`, and `maintain_order` in `$sort_by()` ([#1034](https://github.com/pola-rs/r-polars/issues/1034)).
...
- Other:
  ...
  - All `$write_*()` and `$sink_*()` functions now invisibly return the input
    data ([#1039](https://github.com/pola-rs/r-polars/issues/1039)).
...
- The `parallel` argument of `pl$scan_parquet()` and `pl$read_parquet()` now works
  correctly ([#1033](https://github.com/pola-rs/r-polars/issues/1033)). Previously, any correct value was treated as `"auto"`.
...
- The default value of the `format` of `$str$strptime()` is now correctly set ([#892](https://github.com/pola-rs/r-polars/issues/892)).
...
- `<Expr>$where()` is removed. Use `<Expr>$filter()` instead ([#718](https://github.com/pola-rs/r-polars/issues/718)).
...
- New method `$dt$time()` to extract the time from a `datetime` variable ([#428](https://github.com/pola-rs/r-polars/issues/428)).
